...
'''
def gpu_paillier_encrypt(list,g,r,n):        
    batch_param = []
    batch_param1 = []
    batch_param2 = []
    start = time.time()
    p_bytes = g.to_bytes(256, "little")
    p1_bytes = r.to_bytes(256, "little")
    m_bytes = (n*n).to_bytes(256, "little")
    q1_bytes = n.to_bytes(256, "little")
    for i in list:    
        q_bytes = i.to_bytes(256, "little")
        batch_param.append((p_bytes, q_bytes, m_bytes))
        batch_param1.append((p1_bytes, q1_bytes, m_bytes))
    result = gpu_cal.powm_2048(batch_param, len(batch_param))
    result1 = gpu_cal.powm_2048(batch_param1, len(batch_param1))
    for j,k in zip(result,result1):
        batch_param2.append((j,k, m_bytes))
    result2= gpu_cal.mulm_2048(batch_param2, len(batch_param2))
    # print(int.from_bytes(result2[0], "little"))
...
def gpu_encrypt_vector(public_key, parameters):
    # use gpu's paillier to encrypt the listimage.png
    t = parameters.flatten(0).cpu().numpy().tolist()
    # print(t)
    # a = [1.2,2]
    # print(type(t))
    res = [parameter for parameter in t]
    # print("kukukuku...")
    # print(type(res))

    res = public_key.gpu_encrypt(res)
    return res
...
def gpu_decrypt_vector(private_key, parameters):
    # print(len(parameters))
    res = private_key.gpu_decrypt(parameters)
    return res
...
if __name__=="__main__":
    ...
    # 定义多个参与方，导入训练、测试数据集
    myClients = ClientsGroup(type1, args['IID'], args['num_of_clients'], dev)
    ...
    # print(gpu_encrypt_vector(public_key, [1,2,3]))
    ...
    for i in range(args['num_comm']):
        ...
        for client in tqdm(clients_in_comm):
            ...
            if sum_parameters is None:
                ...
                for key, var in local_parameters.items():
                    ...
                    # print(type(sum_parameters[key]))
                    # print(sum_parameters[key])
                    sum_parameters[key] = gpu_encrypt_vector(public_key, sum_parameters[key])

            else:
                for key in sum_parameters:
                    sum_parameters[key] = np.add(sum_parameters[key], gpu_encrypt_vector(public_key, add_noise(local_parameters[key], dp, dev)))
        print(f'encrypt time:{time.time() - start}')
        ...
        print(f'decrypt time:{time.time() - decrypt_start}')
        # 不进行计算图构建（无需反向传播）
        with torch.no_grad():
            # 满足评估的条件，用测试集进行数据评估
